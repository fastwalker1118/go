        ...
        with np.load(npz_file) as npz:
            ...
            if include_meta:
                if "metadataInputNC" in npz:
                    ...
                    # NPZ lacks metadata (e.g. writetrainingdata_simple output) — fill with zeros
                    n_rows = binaryInputNCHWPacked.shape[0]
                    metadataInputNC = np.zeros((n_rows, 192), dtype=np.float32)
            else:
